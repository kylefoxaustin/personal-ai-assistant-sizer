...
with st.sidebar:
    ...
    tier_name = st.selectbox(
        ...
        help="Low-LP4 = 32-bit LPDDR4 @ 4.266 GT/s, dense INT8-only silicon "
             ...
             "target, 24 GB DRAM, 25 W — primary design point). 200 TOPS "
             "INT8 only (no FP path — Q4_K_M MoE runs via INT8 dequant; "
             "dense fp16-runtime models won't run here without W8A8 "
             "re-quant). "
             "High = 128-bit LPDDR5X @ 8.4 GT/s (same memory class as Mid; "
             "differentiates on COMPUTE — FP-capable silicon at 200 BF16/"
             "FP16 / 400 INT8 / 400 FP8 TOPS, the inflection point where "
             "FP support shows up — and CAPACITY — 32 GB DRAM, 40 W). The "
             "2× INT8 vs FP16 ratio is the standard MAC doubling on the "
             "same hardware. Memory upgrades (LPDDR5T 11.2, LPDDR6 12/14) "
             "are available as overlays on either Mid or High. "
