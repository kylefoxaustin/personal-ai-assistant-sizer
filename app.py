"""Skippy NPU sizer — interactive projection tool (horizontal layout, v2.0.0).
...
Horizontal layout: no left sidebar; controls sit in a horizontal top strip
(tier pills + Model/Workload pickers + Memory/BW-share popovers + a ⚙ Settings
popover for rarely-touched knobs); results + charts fill the full page width;
KPIs are visible onscreen (not download-only); each section's verbose depth-tabs
tuck into a collapsible "🔎 detail" expander so headline metrics stay on a short
first paint. Promoted to primary app.py at v2.0.0 (2026-06-11); the prior
vertical-sidebar layout is preserved as app_vertical_legacy.py. PAI is LLM-only,
so this is the single-workload cut of keyhole-sizer's Vision/LLM/VLA shell.
...
st.set_page_config(page_title="Skippy NPU Sizer",
